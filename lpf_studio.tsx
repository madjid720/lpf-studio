...
Évalue ce titre proposé et réponds UNIQUEMENT en JSON valide (sans markdown ni backticks) :
{
  ...
  "alternatives": [
    ...
    try{
      ...
      setView(id);
    }catch(e){setIdees(prev=>prev.map(i=>i.id===id?{...i,feedback:'Erreur — réessaie.'}:i));}
    setLoading(null);
  };
  ...
  return(
    <div>
      <div style={{fontSize:16,fontWeight:600,color:'#111827',marginBottom:4}}>Workshop — Titres</div>
...
function EspaceScript({scripts}){
  ...
  const buildStyleContext=()=>{
    ...
    const extracts=fallback.map(s=>s.body.slice(0,500)).join('\n---\n');
